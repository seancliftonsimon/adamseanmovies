...
def render():
    ...
    if not filtered:
        st.warning("No movies match those filters. Try broadening your search or add more movies!")
        return

    st.markdown(f"**{len(filtered)}** movies in the pool")
...
def _filter_bar(movies):
    with st.expander("\U0001F527 Filters & Vibes", expanded=False):
        ...
        genre_filter = st.multiselect("Genres", available_genres, key="pick_genre_filter")

        max_runtime = st.slider(
            "Max runtime (minutes)", 60, 300, 300, step=10, key="pick_runtime",
        )
        ...
        for i, (vibe_name, vibe_cfg) in enumerate(VIBE_PRESETS.items()):
            with vibe_cols[i]:
                if st.button(vibe_name, key=f"vibe_{vibe_name}", use_container_width=True):
    ...
    movies = [m for m in movies if (m["runtime"] or 0) <= max_runtime or not m["runtime"]]
...
def _run_reveal(filtered):
    ...
    for i in range(steps):
        ...
        with placeholder.container():
            st.markdown("### \U0001F3AC Finding your perfect movie...")
...
def _show_picked_movie(movie, celebrate=False):
    ...
    with cols[1]:
        ...
        st.caption(f"From: {list_labels.get(movie['list_type'], movie['list_type'])}")

    st.markdown("---")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("\U0001F3B2 Spin Again", key="spin_again", use_container_width=True):
            st.session_state["picked_movie"] = None
            st.rerun()
    with c2:
        if st.button("\u2705 Let's Watch This!", key="lets_watch", type="primary",
                     use_container_width=True):
...
def _watch_form(movie):
    ...
    if st.button("Save to Watch Log", key="pick_save_watch", type="primary",
        ...
        st.toast(f"**{movie['title']}** saved to your Watch Log!", icon="\U0001F37F")
        st.balloons()
        st.rerun()
