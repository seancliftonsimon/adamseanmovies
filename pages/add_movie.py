...
def _do_search(query, who):
    ...
    for movie in results[:12]:
        ...
        with cols[1]:
            if st.button(f"**{title}** ({year})", key=f"sel_{tmdb_id}", use_container_width=True):
...
def _show_detail_and_add(tmdb_id, who):
    ...
    if existing:
        list_labels = {"adam_pick": "Adam's Picks", "sean_pick": "Sean's Picks", "mutual": "Mutual Discoveries"}
        label = list_labels.get(existing["list_type"], existing["list_type"])
        status = "watched" if existing["watched"] else "unwatched"
        st.warning(f"**{existing['title']}** is already in **{label}** ({status}).")
    ...
    added = False
    with col1:
        if st.button("\U0001F3AC Adam's Pick", key="add_adam", use_container_width=True,
                      help="Adam has seen this and wants to share it"):
            added = _save_movie(details, "adam_pick", who)
    with col2:
        if st.button("\U0001F3AC Sean's Pick", key="add_sean", use_container_width=True,
                      help="Sean has seen this and wants to share it"):
            added = _save_movie(details, "sean_pick", who)
    with col3:
        if st.button("\U0001F31F Mutual Discovery", key="add_mutual", use_container_width=True,
...
def _save_movie(details, list_type, added_by):
    ...
    list_labels = {"adam_pick": "Adam's Picks", "sean_pick": "Sean's Picks", "mutual": "Mutual Discoveries"}
    if success:
        st.toast(f"Added **{details['title']}** to {list_labels[list_type]}!", icon="\U0001F37F")
        st.session_state.selected_tmdb_id = None
        st.balloons()
        st.rerun()
    else:
